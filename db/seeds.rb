...
(1..10).each do |g|
    ...
    while !game_finished
        ...
        while !round_finished
            ...
            round_count +=1
            puts "Inicio friends"
            puts "#{players.where(position: current_position)}"
            puts "fin friends"
            ...
            puts "next_player: #{next_player.email}"
            puts "Current position before: #{current_position}"
            current_position = players.where(user_id:next_player.id).first.position
            puts "Current position after: #{current_position}"
